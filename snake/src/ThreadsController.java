...
public class ThreadsController extends Thread {
    ...
    // Задержка между каждым движением змеи
    private void pauser(){
        try {
            sleep(speed);
    ...
    //Проверка, кусает ли змея себя или ест
    ...
    // Остановка игры, при столкновении змеи.
    private void stopTheGame(){
        System.out.println("Cancel_Game! \n");
        while(true){
            pauser();
        }
    }

    // Еда в нужном месте, и показ ее.
    private void spawnFood(Tuple foodPositionIn){
        Squares.get(foodPositionIn.x).get(foodPositionIn.y).lightMeUp(1);
    }

    // Возвращение позиции, не занятую змеей
    ...
    //Перемещает голову змеи и обновляет позиции в arraylist 1:справа 2:слева 3:сверху 4:снизу 0:ничего
    ...
    //Обновляет хвост змеи, удаляя лишние данные в positions arraylist
